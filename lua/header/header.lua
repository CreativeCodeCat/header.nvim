...
local function format_header(ft_config)
    ...
    local SPACES_AFTER_FILENAME = constants.length - (2 * constants.margin) -
                                      string.len(filename) - string.len(dir) -
                                      19
    local SPACES_AFTER_GIT = constants.length - (2 * constants.margin) -
                                 string.len(git) - 19
    local SPACES_AFTER_EMAIL = constants.length - (2 * constants.margin) -
                                   string.len(email) - string.len(username) - 30
    local SPACES_AFTER_CREATED = constants.length - (2 * constants.margin) -
                                     string.len(username) - 52
    local SPACES_AFTER_UPDATED = SPACES_AFTER_CREATED - 1
    -- stylua: ignore
    return string.format(
               constants.header, ft_config.start_comment,
               string.rep(ft_config.fill_comment, 74), ft_config.end_comment,
               ft_config.start_comment, ft_config.end_comment,
               ft_config.start_comment, dir, filename,
               string.rep(" ", SPACES_AFTER_FILENAME), ft_config.end_comment,
               ft_config.start_comment, ft_config.end_comment,
               ft_config.start_comment, username, email,
               string.rep(" ", SPACES_AFTER_EMAIL), ft_config.end_comment,
               ft_config.start_comment, git, string.rep(" ", SPACES_AFTER_GIT),
               ft_config.end_comment, ft_config.start_comment,
               ft_config.end_comment, ft_config.start_comment,
               fn.strftime("%d %b %Y, %I:%M:%S %P"), username,
               string.rep(" ", SPACES_AFTER_CREATED), ft_config.end_comment,
               ft_config.start_comment, fn.strftime("%d %b %Y, %I:%M:%S %P"),
               username, string.rep(" ", SPACES_AFTER_UPDATED),
               ft_config.end_comment, ft_config.start_comment,
               ft_config.end_comment, ft_config.start_comment,
               string.rep(ft_config.fill_comment, 74), ft_config.end_comment
           )
...
header.update = function(ft_config)
    ...
    local found = fn.getline(9):find(
                      ft_config.start_comment ..
                          string.rep(
                              " ", constants.margin - #ft_config.start_comment
                          ) .. "Updated: "
                  )
    if found then
        local filename = fn.expand("%:t")
        ...
        local SPACES_AFTER_FILENAME =
            constants.length - (2 * constants.margin) - string.len(filename) -
                string.len(dir) - 19
        local SPACES_AFTER_UPDATED = constants.length - (2 * constants.margin) -
                                         string.len(username) - 53
